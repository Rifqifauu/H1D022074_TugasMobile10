  ...
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      <div class="scrollable-container">
        <ion-list>
          <ion-item-sliding v-for="todo in activeTodos" :key="todo.id" :ref="(el: any) => setItemRef(el, todo.id!)">
        ...
        <ion-accordion-group>
          <ion-accordion value="first">
            ...
            <div slot="content" class="scrollable-container">
              <ion-list>
                <ion-item-sliding v-for="todo in completedTodos" :key="todo.id" :ref="(el: any) => setItemRef(el, todo.id!)">
                  <ion-item-options side="start" @ionSwipe="handleDelete(todo)">
                    <ion-item-option color="danger" expandable @click="handleDelete(todo)">
                      <ion-icon slot="icon-only" :icon="trash" size="large"></ion-icon>
                    </ion-item-option>
...
import { IonContent, IonHeader, IonPage, IonTitle, IonToolbar, IonItem, IonLabel, IonInput, IonButton, IonList, toastController, loadingController, IonCard, IonAccordionGroup, IonCardTitle, IonCardSubtitle, IonFab } from '@ionic/vue';
import { formatDistanceToNow } from 'date-fns';
import { checkmarkCircle, trash, create, add, close } from 'ionicons/icons';
...
const handleSubmit = async (todo: Omit<Todo, 'id' | 'createdAt' | 'updatedAt' | 'status'>) => {
  if (!todo.title) {
    await showToast('Title is required', 'warning', checkmarkCircle);
    return;
  }
  try {
    if (editingId.value) {
    ...
    await showToast('An error occurred', 'danger', close);
    console.error(error);
  } finally {
...
const handleDelete = async (deleteTodo: Todo) => {
  try {
    ...
    await showToast('Failed to delete todo', 'danger', close);
    console.error(error);
  }
};
...
const handleStatus = async (statusTodo: Todo) => {
  ...
  try {
    ...
    await showToast('Failed to update status', 'danger', close);
    console.error(error);
  }
};
</script>
